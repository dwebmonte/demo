...
class Smarty_Compiler extends Smarty {
    ...
    function _compile_foreach_start($tag_args)
    {
		...
		if (strpos($from,'#')===1) {
			$from = str_ireplace(array('#','"'),'',$from);
			$config_value = $this->get_config_vars($from);	
			if (is_array($config_value)) $output .= "\$_from = \$this->_config[0]['vars']['{$from}'];";
			else $output .= "\$_from = unserialize('{$config_value}');";
		} else $output .= "\$_from = $from;";
