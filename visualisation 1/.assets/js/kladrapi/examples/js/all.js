$(function () {
	...
	(function () {
		...
		var 
			$region = $container.find('[name="region"]'),
			$district = $container.find('[name="district"]'),
			$region_id = $container.find('[name="region_id"]'),
			$district_id = $container.find('[name="district_id"]'),
			$city = $container.find('[name="city"]'),
			$street = $container.find('[name="street"]'),
			$building = $container.find('[name="building"]');
			$block = $container.find('[name="block"]');
			$flat = $container.find('[name="flat"]');
			...
			.kladr({
				...
				change: function (obj) {
					...
					if (obj) {
						...
						// parents
						if (obj.parents) {
							obj.parents.forEach(function(item, i, arr) {
								if (item.contentType == "region") $region.val( item.name );
								if (item.contentType == "district") $district.val( item.name );
							});
						...
						switch (obj.contentType) {
							...
							break;
							//default: console.warn("Unknown contentType = " + contentType);  
						};						
						*/
