	...
	$.ajax({ url: 'index.php', type:'POST', data:send_data, dataType: "json",
		success:function( res ){
			...
			if (typeof callbackFunc === "function") {
				if( callbackFunc( "start", res ) === false ) return false;
			};
...
$(document).ready(function()  {
	...
	$(document).on("click", "[api-href]", function(e) {
		let $this = $(this);
		let api_param = $this.attr("api-param") ? JSON.parse('{' + $this.attr("api-param") + '}') : {};
		let api_handler = $this.attr("api-handler") ? new Function('state', 'data', $this.attr("api-handler")) : undefined;

		
		api_request($this.attr("api-href"), api_param, api_handler);		
